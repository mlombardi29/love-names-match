...
const getMatchLabel = (p1: string, p2: string): { text: string; icon: 'double-like' | 'double-superlike' | 'mixed' } => {
  ...
  if (p1 === 'superlike') {
    return { text: 'Partner 1 Superliked ⭐, Partner 2 Liked ❤️', icon: 'mixed' };
  }
  return { text: 'Partner 1 Liked ❤️, Partner 2 Superliked ⭐', icon: 'mixed' };
};
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {matches.map((match) => {
          ...
          const matchLabel = getMatchLabel(match.partner1Decision, match.partner2Decision);

          return (
            <Card 
              key={match.name.id}
              className={`
                p-5 ${genderStyle.bg} border border-border
